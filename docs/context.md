...
**First-prototype scope cut (deadline-driven, revisit before any lab-deployed build)**: project ships by roughly end of next week (~2026-07-24, stated 2026-07-16) and shipping alone takes a week, so the first board trades robustness for speed. Reverse-polarity protection (the P-channel/N-channel MOSFET pair, polyfuses, TVS clamps on `Battery_Management.sch`) is omitted — raw `±12V_bat` wired directly where the protected `±12VDC_bat` rail would go. The soft on/off DPDT switch is also omitted — `EN` bridged directly to `VIN` on each LDO (U4/U5/U6), a documented standard "always-on" configuration per TI's own datasheets (VEN=VIN is a valid operating condition), not a hack. PG LEDs and the front-panel LED connector (J4) are dropped too — pure status indicators, no functional role. Net effect: the board is always-on whenever powered, unprotected against reversed input, with no on-board power switch — power is controlled at the bench supply. Acceptable for the bench-test/characterization build only; the project's own "operable by any lab member without the engineer present" deliverable goal implies protection and a real power switch belong in whatever board actually ships to the lab.
